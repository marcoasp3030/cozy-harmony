...
const VmPaySettings = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (syncRes.data?.value) {
        setSyncMeta(syncRes.data.value as any);
      }
      setLoading(false);
    });
  }, [user]);
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      if (error) throw error;
      if (data?.success) {
        toast.success("✅ Conexão com VMPay OK!");
      } else {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (data?.success) {
        toast.success(
          `Sincronização concluída! ${data.installations} lojas, ${data.products} produtos importados.`
        );
        setSyncMeta({

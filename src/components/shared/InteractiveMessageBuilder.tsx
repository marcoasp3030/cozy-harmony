...
import { Plus, Trash2, Link, Phone, ListOrdered, MousePointerClick, BarChart3, Type, Settings2 } from "lucide-react";
                ...
                <AccordionItem value="config" className="border-b-0">
                  <AccordionTrigger className="py-2 text-sm font-medium hover:no-underline">
                    <span className="flex items-center gap-2">
                      ...
                      {value.type === "buttons" && `Botões (${value.buttons?.length || 0}/3)`}
                      {value.type === "list" && `Lista (${(value.listSections || []).reduce((a, s) => a + s.rows.length, 0)} itens)`}
                      {value.type === "cta" && `CTA (${value.ctaButtons?.length || 0}/3)`}
                      {value.type === "poll" && `Enquete (${value.pollOptions?.length || 0} opções)`}
                    </span>
                  </AccordionTrigger>
                  <AccordionContent className="space-y-3 pb-3">
                    {/* ─── BUTTONS EDITOR ─── */}

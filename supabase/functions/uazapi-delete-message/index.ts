...
serve(async (req) => {
  ...
  try {
    ...
    const { messageExternalId, instanceId } = await req.json();

    if (!messageExternalId) {
      return new Response(JSON.stringify({ error: 'messageExternalId é obrigatório.' }), {
    ...
    console.error('uazapi-delete-message error:', message);
